...
namespace XenkoToolkit.Engine
{
    ...
    public static class EntityExtensions
    {
        ...
        /// <param name="sceneInstance">The collection of entities.</param>
        ...
        /// <remarks>
        /// You should not use this. Searching all entites from components is not the most performant option.
        /// </remarks>
        public static T FindComponent<T>(this SceneInstance sceneInstance, bool includeDisabled = false) where T : EntityComponent
        {
            if (sceneInstance == null)
                throw new ArgumentNullException(nameof(sceneInstance));

            return sceneInstance.SelectMany(e => e.Components)
                                .OfType<T>()
                                .Where(c=> includeDisabled || ((c as ActivableEntityComponent)?.Enabled).GetValueOrDefault(true))
                                .FirstOrDefault();
